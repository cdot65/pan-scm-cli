...
class Address(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=63, description="Name of the address object"
    )
    ...
    def validate_address_type(self) -> "Address":
        ...
        if len(provided) == 0:
            ...
            raise ValueError(
                "Only one of 'ip_netmask', 'ip_range', 'ip_wildcard', or 'fqdn' can be provided."
            )
...
class Application(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=63, description="Name of the application"
    )
    ...
    description: str = Field(
        "", max_length=1023, description="Description of the application"
    )
    ports: list[str] = Field(
        default_factory=list, description="Associated TCP/UDP ports"
    )
    ...
    has_known_vulnerabilities: bool = Field(
        False, description="Has known vulnerabilities"
    )
...
class ApplicationGroup(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=63, description="Name of the application group"
    )
    members: list[str] = Field(
        ..., min_length=1, description="List of application names"
    )
...
class ApplicationFilter(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=63, description="Name of the application filter"
    )
    category: list[str] = Field(
        ..., min_length=1, description="List of category strings"
    )
    subcategory: list[str] = Field(
        ..., min_length=1, description="List of subcategory strings"
    )
    technology: list[str] = Field(
        ..., min_length=1, description="List of technology strings"
    )
    risk: list[int] = Field(..., min_length=1, description="List of risk levels (1-5)")
    evasive: bool = Field(
        False, description="Filter for apps that use evasive techniques"
    )
    pervasive: bool = Field(False, description="Filter for apps that are widely used")
    excessive_bandwidth_use: bool = Field(
        False, description="Filter for apps that use excessive bandwidth"
    )
    used_by_malware: bool = Field(False, description="Filter for apps used by malware")
    transfers_files: bool = Field(
        False, description="Filter for apps that transfer files"
    )
    has_known_vulnerabilities: bool = Field(
        False, description="Filter for apps with known vulnerabilities"
    )
    tunnels_other_apps: bool = Field(
        False, description="Filter for apps that tunnel other applications"
    )
    prone_to_misuse: bool = Field(False, description="Filter for apps prone to misuse")
    no_certifications: bool = Field(
        False, description="Filter for apps lacking certifications"
    )
    ...
    def validate_risk_values(self) -> "ApplicationFilter":
        ...
        for risk_value in self.risk:
            if risk_value < 1 or risk_value > 5:
                raise ValueError(
                    f"Risk value {risk_value} is out of range. Must be between 1 and 5."
                )
        return self
...
class DynamicUserGroup(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=63, description="Name of the dynamic user group"
    )
    filter: str = Field(..., max_length=2047, description="Tag-based filter expression")
    description: str = Field(
        "", max_length=1023, description="Description of the dynamic user group"
    )
    tags: list[str] = Field(
        default_factory=list, description="Tags associated with the dynamic user group"
    )
...
class ExternalDynamicList(BaseModel):
    ...
    description: str = Field(
        "", max_length=255, description="Description of the external dynamic list"
    )
    exception_list: list[str] = Field(
        default_factory=list, description="Exception list entries"
    )
    ...
    day: str | None = Field(
        None, description="Day for weekly (sunday-saturday) or monthly (1-31) updates"
    )

    # Authentication
    username: str | None = Field(
        None, max_length=255, description="Authentication username"
    )
    password: str | None = Field(
        None, max_length=255, description="Authentication password"
    )
    certificate_profile: str | None = Field(
        None, description="Certificate profile for authentication"
    )

    # Domain-specific
    expand_domain: bool = Field(
        False, description="Enable/Disable expand domain (for domain type)"
    )
    ...
    def validate_edl_type(self) -> "ExternalDynamicList":
        ...
        if self.type not in valid_types:
            raise ValueError(
                f"Invalid EDL type '{self.type}'. Must be one of: {', '.join(valid_types)}"
            )

        # Custom EDLs require recurring configuration
        if self.type in ["ip", "domain", "url", "imsi", "imei"] and not self.recurring:
            raise ValueError(
                f"EDL type '{self.type}' requires 'recurring' configuration"
            )
        ...
        if self.recurring:
            if self.recurring in ["daily", "weekly", "monthly"] and not self.hour:
                raise ValueError(
                    f"Recurring '{self.recurring}' requires 'hour' to be set"
                )
            if self.recurring == "weekly" and not self.day:
                raise ValueError(
                    "Recurring 'weekly' requires 'day' to be set (sunday-saturday)"
                )
        ...
        if self.type in ["predefined_ip", "predefined_url"]:
            ...
            if self.recurring == "five_minute":
                ...
                type_config["recurring"] = {
                    "weekly": {"day_of_week": self.day, "at": self.hour}
                }
...
class HIPObject(BaseModel):
    ...
    description: str = Field(
        "", max_length=255, description="Description of the HIP object"
    )

    # Host information criteria
    host_info_domain: str | None = Field(
        None, description="Domain criteria (is, is_not, contains)"
    )
    host_info_domain_value: str | None = Field(
        None, max_length=255, description="Domain value to match"
    )
    host_info_os: str | None = Field(
        None, description="OS vendor (Microsoft, Apple, Google, Linux, Other)"
    )
    host_info_os_value: str | None = Field(
        None, max_length=255, description="OS value (All or specific version)"
    )
    host_info_client_version: str | None = Field(
        None, description="Client version criteria (is, is_not, contains)"
    )
    host_info_client_version_value: str | None = Field(
        None, max_length=255, description="Client version value"
    )
    host_info_host_name: str | None = Field(
        None, description="Host name criteria (is, is_not, contains)"
    )
    host_info_host_name_value: str | None = Field(
        None, max_length=255, description="Host name value"
    )
    host_info_host_id: str | None = Field(
        None, description="Host ID criteria (is, is_not, contains)"
    )
    host_info_host_id_value: str | None = Field(
        None, max_length=255, description="Host ID value"
    )
    host_info_managed: bool | None = Field(None, description="Managed state criteria")
    host_info_serial_number: str | None = Field(
        None, description="Serial number criteria (is, is_not, contains)"
    )
    host_info_serial_number_value: str | None = Field(
        None, max_length=255, description="Serial number value"
    )

    # Network information
    network_info_type: str | None = Field(None, description="Network type (is, is_not)")
    network_info_value: str | None = Field(
        None, description="Network value (wifi, mobile, ethernet, unknown)"
    )

    # Patch management
    patch_management_enabled: bool | None = Field(
        None, description="Whether patch management is enabled"
    )
    patch_management_missing_patches: str | None = Field(
        None, description="Missing patches check (has-any, has-none, has-all)"
    )
    patch_management_severity: int | None = Field(
        None, ge=0, le=100000, description="Patch severity level"
    )
    patch_management_patches: list[str] | None = Field(
        None, description="List of specific patches"
    )
    patch_management_vendors: list[dict[str, Any]] | None = Field(
        None, description="Vendor specifications"
    )

    # Disk encryption
    disk_encryption_enabled: bool | None = Field(
        None, description="Whether disk encryption is enabled"
    )
    disk_encryption_locations: list[dict[str, Any]] | None = Field(
        None, description="Encryption location specifications"
    )
    disk_encryption_vendors: list[dict[str, Any]] | None = Field(
        None, description="Vendor specifications"
    )

    # Mobile device
    mobile_device_jailbroken: bool | None = Field(None, description="Jailbroken status")
    mobile_device_disk_encrypted: bool | None = Field(
        None, description="Disk encryption status"
    )
    mobile_device_passcode_set: bool | None = Field(None, description="Passcode status")
    mobile_device_last_checkin_time: str | None = Field(
        None, description="Last check-in time type (days, hours)"
    )
    mobile_device_last_checkin_value: int | None = Field(
        None, ge=1, le=65535, description="Last check-in time value"
    )
    mobile_device_has_malware: bool | None = Field(None, description="Malware presence")
    mobile_device_has_unmanaged_app: bool | None = Field(
        None, description="Unmanaged apps presence"
    )
    mobile_device_applications: list[dict[str, Any]] | None = Field(
        None, description="Application specifications"
    )

    # Certificate
    certificate_profile: str | None = Field(
        None, description="Certificate profile name"
    )
    certificate_attributes: list[dict[str, Any]] | None = Field(
        None, description="Certificate attribute specifications"
    )
    ...
    def validate_criteria_pairs(self) -> "HIPObject":
        ...
        if (
            ...
            and not self.mobile_device_last_checkin_value
        ):
            raise ValueError(
                "mobile_device_last_checkin_time requires mobile_device_last_checkin_value"
            )
        if (
            self.mobile_device_last_checkin_value
            and not self.mobile_device_last_checkin_time
        ):
            raise ValueError(
                "mobile_device_last_checkin_value requires mobile_device_last_checkin_time"
            )
    ...
    def to_sdk_model(self) -> dict[str, Any]:
        ...
        if any(
            ...
            if self.host_info_client_version and self.host_info_client_version_value:
                if self.host_info_client_version == "is":
                    criteria["client_version"] = {
                        "is": self.host_info_client_version_value
                    }
                elif self.host_info_client_version == "is_not":
                    criteria["client_version"] = {
                        "is_not": self.host_info_client_version_value
                    }
                elif self.host_info_client_version == "contains":
                    criteria["client_version"] = {
                        "contains": self.host_info_client_version_value
                    }
            ...
            if self.host_info_serial_number and self.host_info_serial_number_value:
                if self.host_info_serial_number == "is":
                    criteria["serial_number"] = {
                        "is": self.host_info_serial_number_value
                    }
                elif self.host_info_serial_number == "is_not":
                    criteria["serial_number"] = {
                        "is_not": self.host_info_serial_number_value
                    }
                elif self.host_info_serial_number == "contains":
                    criteria["serial_number"] = {
                        "contains": self.host_info_serial_number_value
                    }
        ...
        if any(
            ...
            if (
                self.mobile_device_last_checkin_time
                and self.mobile_device_last_checkin_value
            ):
            ...
            if (
                self.mobile_device_has_malware is not None
                or self.mobile_device_has_unmanaged_app is not None
                or self.mobile_device_applications
            ):
                applications = {}
                ...
                if self.mobile_device_has_unmanaged_app is not None:
                    applications["has_unmanaged_app"] = (
                        self.mobile_device_has_unmanaged_app
                    )
...
class HIPProfile(BaseModel):
    ...
    description: str | None = Field(
        None, max_length=255, description="Description of the HIP profile"
    )
    match: str = Field(
        ..., max_length=2048, description="Match criteria for the HIP profile"
    )
...
class HTTPServerProfile(BaseModel):
    ...
    description: str | None = Field(
        None, description="Description of the HTTP server profile"
    )
    ...
    def validate_servers(self) -> "HTTPServerProfile":
        ...
        for idx, server in enumerate(self.servers):
            ...
            if (
                ...
                and "tls_version" in server
                and server["tls_version"] not in ["1.0", "1.1", "1.2", "1.3"]
            ):
                raise ValueError(
                    f"Server {idx}: tls_version must be one of: 1.0, 1.1, 1.2, 1.3"
                )
            ...
            if "http_method" in server and server["http_method"] not in [
                ...
                raise ValueError(
                    f"Server {idx}: http_method must be one of: GET, POST, PUT, DELETE"
                )
...
class LogForwardingProfile(BaseModel):
    ...
    description: str | None = Field(
        None, max_length=255, description="Description of the log forwarding profile"
    )
    enhanced_application_logging: bool = Field(
        False, description="Enable enhanced application logging"
    )
        ...
        if self.match_list:
            for idx, match in enumerate(self.match_list):
                ...
                if not any(match.get(action) for action in actions):
                    raise ValueError(
                        f"Match list {idx}: At least one action must be specified (send_http, send_syslog, send_to_panorama, or quarantine)"
                    )
    ...
    def to_sdk_model(self) -> dict[str, Any]:
        ...
        if self.enhanced_application_logging:
            model_data["enhanced_application_logging"] = (
                self.enhanced_application_logging
            )
...
class Service(BaseModel):
    ...
    description: str | None = Field(
        None, max_length=1023, description="Description of the service"
    )
    ...
    def validate_service(self) -> "Service":
        ...
        if "port" not in protocol_config:
            raise ValueError(
                f"Port configuration is required for {protocol_type.upper()}"
            )
            ...
            if "-" in port:
                ...
                try:
                    ...
                    if "invalid literal" in str(e):
                        raise ValueError(
                            "Port range must contain valid integers"
                        ) from e
                    raise
            elif "," in port:
        ...
        if "override" in protocol_config:
            ...
            if "halfclose_timeout" in override:
                ...
                if not isinstance(halfclose, int) or halfclose < 0:
                    raise ValueError(
                        "Override halfclose_timeout must be a non-negative integer"
                    )
            if "timewait_timeout" in override:
                timewait = override["timewait_timeout"]
                if not isinstance(timewait, int) or timewait < 0:
                    raise ValueError(
                        "Override timewait_timeout must be a non-negative integer"
                    )
        ...
        if v is not None:
            ...
            for container in other_containers:
                if values.get(container) is not None:
                    raise ValueError(
                        "Exactly one of 'folder', 'snippet', or 'device' must be set"
                    )
    ...
    def validate_servers(
        ...
        for server in v:
            ...
            if server["facility"] not in valid_facilities:
                raise ValueError(
                    f"Facility must be one of: {', '.join(valid_facilities)}"
                )
...
class Tag(BaseModel):
    ...
    comments: str | None = Field(
        None, description="Comments for the tag", max_length=1023
    )
    snippet: str | None = Field(None, description="Snippet location")
    device: str | None = Field(None, description="Device location")
...
class Zone(BaseModel):
    ...
    snippet: str | None = Field(None, description="Snippet location")
    device: str | None = Field(None, description="Device location")
    enable_user_identification: bool | None = Field(
        None, description="Enable user identification"
    )
    enable_device_identification: bool | None = Field(
        None, description="Enable device identification"
    )
...
class SecurityRule(BaseModel):
    ...
    source_zones: list[str] = Field(
        default_factory=lambda: ["any"], description="List of source zones"
    )
    destination_zones: list[str] = Field(
        default_factory=lambda: ["any"], description="List of destination zones"
    )
    source_addresses: list[str] = Field(
        default_factory=lambda: ["any"], description="List of source addresses"
    )
    destination_addresses: list[str] = Field(
        default_factory=lambda: ["any"], description="List of destination addresses"
    )
    applications: list[str] = Field(
        default_factory=lambda: ["any"], description="List of applications"
    )
    service: list[str] = Field(
        default_factory=lambda: ["any"], description="List of services"
    )
    action: str = Field("allow", description="Action to take")
    description: str | None = Field(
        None, description="Description of the security rule"
    )
    tags: list[str] | None = Field(None, description="List of tags")
    ...
    destination_hip: list[str] | None = Field(
        None, description="Destination HIP profiles"
    )
...
class AntiSpywareProfile(BaseModel):
    ...
    folder: str | None = Field(
        None, description="Folder path for the anti-spyware profile"
    )
    snippet: str | None = Field(
        None, description="Snippet path for the anti-spyware profile"
    )
    device: str | None = Field(
        None, description="Device path for the anti-spyware profile"
    )
    name: str = Field(..., description="Name of the anti-spyware profile")
    description: str | None = Field(
        None, description="Description of the anti-spyware profile"
    )

    # Threat exceptions
    threat_exceptions: list[dict[str, Any]] | None = Field(
        None, description="List of threat exceptions"
    )

    # Rules configuration
    rules: list[dict[str, Any]] | None = Field(
        None, description="List of anti-spyware rules"
    )

    # MICA engine settings
    mica_engine_spyware_enabled: list[dict[str, Any]] | None = Field(
        None, description="MICA engine spyware detection settings"
    )

    # Cloud inline analysis
    cloud_inline_analysis: bool | None = Field(
        None, description="Enable cloud inline analysis"
    )
...
class DecryptionProfile(BaseModel):
    ...
    folder: str | None = Field(
        None, description="Folder path for the decryption profile"
    )
    snippet: str | None = Field(
        None, description="Snippet path for the decryption profile"
    )
    device: str | None = Field(
    ...
    ssl_forward_proxy: dict[str, Any] | None = Field(
        None, description="SSL Forward Proxy settings"
    )

    # SSL Inbound Proxy settings
    ssl_inbound_proxy: dict[str, Any] | None = Field(
        None, description="SSL Inbound Proxy settings"
    )

    # SSL No Proxy settings
    ssl_no_proxy: dict[str, Any] | None = Field(
        None, description="SSL No Proxy settings"
    )

    # SSL Protocol Settings
    ssl_protocol_settings: dict[str, Any] | None = Field(
        None, description="SSL Protocol settings"
    )
    ...
    def validate_proxy_settings(self) -> "DecryptionProfile":
        ...
        if not any(proxy_types):
            raise ValueError(
                "At least one proxy type (ssl_forward_proxy, ssl_inbound_proxy, or ssl_no_proxy) must be configured"
            )
        return self

    @field_validator("ssl_protocol_settings")
    def validate_ssl_protocol_settings(
        cls, v: dict[str, Any] | None
    ) -> dict[str, Any] | None:  # noqa: N805
        ...
        if "min_version" in v and "max_version" in v:
            ...
            min_idx = (
                ssl_versions.index(v["min_version"])
                if v["min_version"] in ssl_versions
                else -1
            )
            max_idx = (
                ssl_versions.index(v["max_version"])
                if v["max_version"] in ssl_versions
                else -1
            )
...
def validate_yaml_file(
    data: dict[str, Any], model_class: type[ModelT], key: str
) -> list[ModelT]:
