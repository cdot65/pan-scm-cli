...
class SecurityZone(BaseObject):
    ...
    def _apply_filters(
        ...
        if "enable_user_identification" in filters:
            ...
            filter_criteria = [zone for zone in filter_criteria if zone.enable_user_identification == enable_user_id]
        ...
        if "enable_device_identification" in filters:
            ...
            filter_criteria = [zone for zone in filter_criteria if zone.enable_device_identification == enable_device_id]
        ...
        if "network_type" in filters:
            ...
            filter_criteria = [zone for zone in filter_criteria if zone.network and any(getattr(zone.network, network_type) is not None for network_type in network_types)]
    ...
    def fetch(
        ...
        # Handle the alternate format (like list() with 'data' array)
            ...
            if len(response["data"]) > 1:
                self.logger.warning(f"Multiple security zones found for '{name}'. Using the first one.")
