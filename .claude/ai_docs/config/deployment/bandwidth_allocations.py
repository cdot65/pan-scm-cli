...
class BandwidthAllocations(BaseObject):
    ...
    def _apply_filters(
        ...
        if "name" in filters:
            ...
            if isinstance(name_filter, list):
                ...
                if not name_filter:
                    return []
                filtered_allocations = [alloc for alloc in filtered_allocations if alloc.name.lower() in [n.lower() for n in name_filter]]
            else:
                filtered_allocations = [alloc for alloc in filtered_allocations if alloc.name.lower() == name_filter.lower()]
        ...
        if "allocated_bandwidth" in filters:
            ...
            if isinstance(bw_filter, list):
                ...
                if not bw_filter:
                    return []
                filtered_allocations = [alloc for alloc in filtered_allocations if alloc.allocated_bandwidth in bw_filter]
            else:
                filtered_allocations = [alloc for alloc in filtered_allocations if alloc.allocated_bandwidth == bw_filter]
        ...
        if "spn_name_list" in filters:
            ...
            if isinstance(spn_filter, list):
                ...
                filtered_allocations = [alloc for alloc in filtered_allocations if alloc.spn_name_list and any(spn in alloc.spn_name_list for spn in spn_filter)]
            else:
                # Single string => check if it's in the allocation's spn_name_list
                filtered_allocations = [alloc for alloc in filtered_allocations if alloc.spn_name_list and spn_filter in alloc.spn_name_list]
        ...
        if "qos_enabled" in filters:
            ...
            filtered_allocations = [alloc for alloc in filtered_allocations if alloc.qos and alloc.qos.enabled == qos_enabled]
    ...
    def get(
        self,
        name: str,
    ) -> BandwidthAllocationResponseModel | None:
