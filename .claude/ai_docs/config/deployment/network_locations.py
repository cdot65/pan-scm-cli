...
class NetworkLocations(BaseObject):
    ...
    def _validate_max_limit(self, limit: int | None) -> int:
    ...
    def _apply_filters(
        ...
        if "value" in filters:
            ...
            if isinstance(value_filter, list):
                ...
                if not value_filter:
                    return []
                filtered_locations = [loc for loc in filtered_locations if loc.value.lower() in [v.lower() for v in value_filter]]
            else:
                filtered_locations = [loc for loc in filtered_locations if loc.value.lower() == value_filter.lower()]
        ...
        if "display" in filters:
            ...
            if isinstance(display_filter, list):
                ...
                if not display_filter:
                    return []
                filtered_locations = [loc for loc in filtered_locations if loc.display.lower() in [d.lower() for d in display_filter]]
            else:
                filtered_locations = [loc for loc in filtered_locations if loc.display.lower() == display_filter.lower()]
        ...
        if "region" in filters:
            ...
            if isinstance(region_filter, list):
                ...
                if not region_filter:
                    return []
                filtered_locations = [loc for loc in filtered_locations if loc.region and loc.region.lower() in [r.lower() for r in region_filter]]
            else:
                filtered_locations = [loc for loc in filtered_locations if loc.region and loc.region.lower() == region_filter.lower()]
        ...
        if "continent" in filters:
            ...
            if isinstance(continent_filter, list):
                ...
                if not continent_filter:
                    return []
                filtered_locations = [loc for loc in filtered_locations if loc.continent and loc.continent.lower() in [c.lower() for c in continent_filter]]
            else:
                filtered_locations = [loc for loc in filtered_locations if loc.continent and loc.continent.lower() == continent_filter.lower()]
        ...
        if "aggregate_region" in filters:
            ...
            if isinstance(agg_region_filter, list):
                ...
                if not agg_region_filter:
                    return []
                filtered_locations = [loc for loc in filtered_locations if loc.aggregate_region and loc.aggregate_region.lower() in [ar.lower() for ar in agg_region_filter]]
            else:
                filtered_locations = [loc for loc in filtered_locations if loc.aggregate_region and loc.aggregate_region.lower() == agg_region_filter.lower()]
    ...
    def fetch(
        ...
        if len(matching_locations) > 1:
            self.logger.warning(f"Multiple network locations found with value '{value}'. Using the first one.")
