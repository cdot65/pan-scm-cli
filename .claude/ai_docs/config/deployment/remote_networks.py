    ...
    def _apply_filters(
        ...
        if "subnets" in filters:
            ...
            if isinstance(subnets_value, list):
                ...
                filtered = [rn for rn in filtered if has_subnet_overlap(rn.subnets or [], subnets_value)]
            else:
        ...
        if "ecmp_load_balancing" in filters:
            ...
            filtered = [rn for rn in filtered if rn.ecmp_load_balancing is not None and match_field(rn.ecmp_load_balancing.value, ecmp_value)]
        ...
        if "protocol" in filters:
            ...
            if isinstance(protocol_value, list):
                ...
                filtered = [rn for rn in filtered if rn.protocol and rn.protocol.bgp and rn.protocol.bgp.enable and "bgp" in protocol_value]
            else:
                # Single string => we do the same idea. This is just an example:
                if protocol_value == "bgp":
                    filtered = [rn for rn in filtered if rn.protocol and rn.protocol.bgp and rn.protocol.bgp.enable]
                else:
