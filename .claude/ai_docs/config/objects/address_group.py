...
class AddressGroup(BaseObject):
    ...
    def _apply_filters(
        ...
        if "types" in filters:
            ...
            filter_criteria = [group for group in filter_criteria if any(getattr(group, field) is not None for field in ["static", "dynamic"] if field in types)]
        ...
        if "values" in filters:
            ...
            filter_criteria = [group for group in filter_criteria if (group.static and any(value in group.static for value in values)) or (group.dynamic and group.dynamic.filter in values)]
        ...
        if "tags" in filters:
            ...
            tags = filters["tags"]
            filter_criteria = [group for group in filter_criteria if group.tag and any(tag in group.tag for tag in tags)]
