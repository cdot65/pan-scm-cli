...
class Address(BaseObject):
    ...
    def _apply_filters(
        ...
        if "types" in filters:
            ...
            filter_criteria = [
                addr for addr in filter_criteria if any(getattr(addr, field) is not None for field in ["ip_netmask", "ip_range", "ip_wildcard", "fqdn"] if field.replace("ip_", "") in types)
            ]
        ...
        if "values" in filters:
            ...
            filter_criteria = [
                addr for addr in filter_criteria if any(getattr(addr, field) in values for field in ["ip_netmask", "ip_range", "ip_wildcard", "fqdn"] if getattr(addr, field) is not None)
            ]
        ...
        if "tags" in filters:
            ...
            tags = filters["tags"]
            filter_criteria = [addr for addr in filter_criteria if addr.tag and any(tag in addr.tag for tag in tags)]
