...
class Schedule(BaseObject):
    ...
    def _apply_filters(
        ...
        if "schedule_type" in filters:
            ...
            filtered_schedules = [s for s in filtered_schedules if hasattr(s.schedule_type, schedule_type) and getattr(s.schedule_type, schedule_type) is not None]
        ...
        if "recurring_type" in filters:
            ...
            filtered_schedules = [
                ...
                and ((recurring_type == "weekly" and s.schedule_type.recurring.weekly is not None) or (recurring_type == "daily" and s.schedule_type.recurring.daily is not None))
            ]
    ...
    def list(
        ...
        **filters,
        ...
        if exact_match and container_parameters:
            ...
            filtered_objects = [obj for obj in filtered_objects if getattr(obj, container_key) == container_value]

        # Exclude folders if provided
        if exclude_folders and isinstance(exclude_folders, list):
            filtered_objects = [obj for obj in filtered_objects if obj.folder not in exclude_folders]

        # Exclude snippets if provided
        if exclude_snippets and isinstance(exclude_snippets, list):
            filtered_objects = [obj for obj in filtered_objects if obj.snippet not in exclude_snippets]

        # Exclude devices if provided
        if exclude_devices and isinstance(exclude_devices, list):
            filtered_objects = [obj for obj in filtered_objects if obj.device not in exclude_devices]
