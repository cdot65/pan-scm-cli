...
class SyslogServerProfile(BaseObject):
    ...
    def _apply_filters(
        ...
        if "transport" in filters:
            ...
            filter_criteria = [profile for profile in filter_criteria if any(server_data.get("transport") in transports for server_name, server_data in profile.servers.items())]
        ...
        if "format" in filters:
            ...
            filter_criteria = [profile for profile in filter_criteria if any(server_data.get("format") in formats for server_name, server_data in profile.servers.items())]
