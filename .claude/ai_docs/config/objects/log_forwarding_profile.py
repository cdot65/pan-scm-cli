...
class LogForwardingProfile(BaseObject):
    ...
    def create(
        ...
        try:
            ...
            self.logger.error(f"Error in API call to create log forwarding profile: {str(e)}", exc_info=True)
            raise
    ...
    def _apply_filters(
        ...
        if "log_type" in filters:
            ...
            # Apply the filter
            filter_criteria = [profile for profile in filter_criteria if profile.match_list and any(match.log_type in log_type_filter for match in profile.match_list)]
        ...
        if "log_types" in filters:
            ...
            filter_criteria = [profile for profile in filter_criteria if profile.match_list and any(match.log_type in log_types for match in profile.match_list)]
        ...
        if "tags" in filters:
            ...
            tags = filters["tags"]
            filter_criteria = [profile for profile in filter_criteria if hasattr(profile, "tag") and profile.tag and any(tag in profile.tag for tag in tags)]
