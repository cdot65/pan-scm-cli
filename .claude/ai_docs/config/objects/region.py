...
class Region(BaseObject):
    ...
    def _apply_filters(
        ...
        if "geo_location" in filters:
            ...
            if "latitude" in geo_filter:
                ...
                if isinstance(lat_range, dict) and "min" in lat_range and "max" in lat_range:
                    filter_criteria = [
                        region for region in filter_criteria if region.geo_location and region.geo_location.latitude >= lat_range["min"] and region.geo_location.latitude <= lat_range["max"]
                    ]
            ...
            if "longitude" in geo_filter:
                ...
                if isinstance(long_range, dict) and "min" in long_range and "max" in long_range:
                    filter_criteria = [
                        ...
                        if region.geo_location and region.geo_location.longitude >= long_range["min"] and region.geo_location.longitude <= long_range["max"]
                    ]
        ...
        if "addresses" in filters:
            ...
            filter_criteria = [region for region in filter_criteria if region.address and any(addr in region.address for addr in addresses)]
