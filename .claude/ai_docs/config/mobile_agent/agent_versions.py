...
class AgentVersions(BaseObject):
        ...
        if "version" in filters:
            ...
            if isinstance(version_filter, list):
                ...
                if not version_filter:
                    return []
                filtered_versions = [ver for ver in filtered_versions if any(v.lower() in ver.lower() for v in version_filter)]
            else:
                filtered_versions = [ver for ver in filtered_versions if version_filter.lower() in ver.lower()]
        ...
        if "prefix" in filters:
            ...
            if isinstance(prefix_filter, list):
                ...
                if not prefix_filter:
                    return []
                filtered_versions = [ver for ver in filtered_versions if any(ver.lower().startswith(p.lower()) for p in prefix_filter)]
            else:
                filtered_versions = [ver for ver in filtered_versions if ver.lower().startswith(prefix_filter.lower())]
    ...
    def fetch(
        ...
        if len(matching_versions) > 1:
            self.logger.warning(f"Multiple agent versions found matching '{version}'. Using the first one.")
