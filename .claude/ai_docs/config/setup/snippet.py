...
class Snippet(BaseObject):
    ...
    def __init__(
        self,
        api_client,
        max_limit: int | None = DEFAULT_MAX_LIMIT,
    ):
    ...
    def _apply_filters(
        ...
        if "labels" in filters:
            ...
            if required_labels:  # Only filter if labels list is not empty
                filtered_data = [item for item in filtered_data if item.labels and required_labels.intersection(set(item.labels))]

        # Filter by types (client-side)
        if "types" in filters:
            required_types = filters["types"]
            # Validate that required_types is a list of strings
            if not isinstance(required_types, list) or not all(isinstance(t, str) for t in required_types):
                raise InvalidObjectError(
            ...
            if required_types:  # Only filter if the list is not empty
                ...
                filtered_data = [item for item in filtered_data if item.type and item.type in required_types_set]
    ...
    def associate_folder(self, snippet_id: str | UUID, folder_id: str | UUID) -> SnippetResponseModel:
        ...
        try:
            ...
            raise NotImplementedError(f"Associating snippets with folders is not yet implemented: {str(e)}")

    def disassociate_folder(self, snippet_id: str | UUID, folder_id: str | UUID) -> SnippetResponseModel:
        ...
        try:
            response = self.api_client.delete(f"{self.ENDPOINT}/{snippet_id_str}/folders/{folder_id_str}")
            return SnippetResponseModel.model_validate(response)
        except Exception as e:
            raise NotImplementedError(f"Disassociating snippets from folders is not yet implemented: {str(e)}")
