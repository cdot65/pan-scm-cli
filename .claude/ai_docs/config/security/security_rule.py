...
class SecurityRule(BaseObject):
    ...
    def _apply_filters(
        ...
        if "category" in filters:
            ...
            filter_criteria = [rule for rule in filter_criteria if any(cat in rule.category for cat in categories)]
        ...
        if "service" in filters:
            ...
            filter_criteria = [rule for rule in filter_criteria if any(svc in rule.service for svc in services)]
        ...
        if "application" in filters:
            ...
            filter_criteria = [rule for rule in filter_criteria if any(app in rule.application for app in applications)]
        ...
        if "destination" in filters:
            ...
            filter_criteria = [rule for rule in filter_criteria if any(dest in rule.destination for dest in destinations)]
        ...
        if "to_" in filters:
            ...
            filter_criteria = [rule for rule in filter_criteria if any(zone in rule.to_ for zone in to_zones)]
        ...
        if "source" in filters:
            ...
            filter_criteria = [rule for rule in filter_criteria if any(src in rule.source for src in sources)]
        ...
        if "from_" in filters:
            ...
            filter_criteria = [rule for rule in filter_criteria if any(zone in rule.from_ for zone in from_zones)]
        ...
        if "tag" in filters:
            ...
            tags = filters["tag"]
            filter_criteria = [rule for rule in filter_criteria if rule.tag and any(tag in rule.tag for tag in tags)]
        ...
        if "profile_setting" in filters:
            ...
            filter_criteria = [rule for rule in filter_criteria if rule.profile_setting and rule.profile_setting.group and any(group in rule.profile_setting.group for group in groups)]
    ...
    def _build_container_params(
        ...
        return {k: v for k, v in {"folder": folder, "snippet": snippet, "device": device}.items() if v is not None}
    ...
    def list(
        ...
        **filters,
        ...
        if exact_match:
            filtered_objects = [each for each in filtered_objects if getattr(each, container_key) == container_value]
