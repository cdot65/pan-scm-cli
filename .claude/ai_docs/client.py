...
class Scm:
    ...
    def __init__(
        ...
        if access_token:
            ...
            self.logger.debug(f"Session created with bearer token: {self.session.headers}, verify_ssl={self.verify_ssl}")
            return

        # OAuth2 client credentials flow authentication mode
        if not all([client_id, client_secret, tsg_id]):
            raise APIError("When not using access_token, client_id, client_secret, and tsg_id are required")
    ...
    def wait_for_job(self, job_id: str, timeout: int = 300, poll_interval: int = 10) -> JobStatusResponse | None:
    ...
    def commit(
        ...
        if admin is None:
            if self.oauth_client:
                ...
                raise APIError("When using bearer token authentication, 'admin' must be provided for commit operations")
