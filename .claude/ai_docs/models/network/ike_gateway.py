...
class CertificateAuth(BaseModel):
    ...
    allow_id_payload_mismatch: bool | None = Field(
        None,
    ...
    local_certificate: dict[str, Any] | None = Field(
        None,
        description="Local certificate configuration",
    )
    strict_validation_revocation: bool | None = Field(
        None,
        description="Enable strict validation revocation",
    )
    use_management_as_source: bool | None = Field(
        None,
...
class Authentication(BaseModel):
    ...
    pre_shared_key: PreSharedKey | None = Field(
        None,
        description="Pre-shared key authentication",
    )
    certificate: CertificateAuth | None = Field(
        None,
    ...
    def validate_auth_method(self) -> "Authentication":
        ...
        if self.pre_shared_key is not None and self.certificate is not None:
            raise ValueError("Only one authentication method can be configured: pre_shared_key or certificate")
        if self.pre_shared_key is None and self.certificate is None:
            raise ValueError("At least one authentication method must be provided: pre_shared_key or certificate")
        return self
...
class IKEv1(BaseModel):
    ...
    dpd: DeadPeerDetection | None = Field(
        None,
...
class Protocol(BaseModel):
    ...
    version: ProtocolVersion | None = Field(
...
class ProtocolCommon(BaseModel):
    ...
    nat_traversal: NatTraversal | None = Field(
        None,
        description="NAT traversal configuration",
    )
    passive_mode: bool | None = Field(
        None,
        description="Enable passive mode",
    )
    fragmentation: Fragmentation | None = Field(
        None,
...
class PeerAddress(BaseModel):
    ...
    dynamic: dict[str, Any] | None = Field(
        None,
        description="Dynamic peer address configuration",
    )
    ...
    def validate_peer_address(self) -> "PeerAddress":
        ...
        if len(filled_types) != 1:
            raise ValueError("Exactly one peer address type must be configured: ip, fqdn, or dynamic")
        return self
...
class IKEGatewayBaseModel(BaseModel):
    ...
    peer_id: PeerId | None = Field(
        None,
        description="Peer identification",
    )
    local_id: LocalId | None = Field(
        None,
    ...
    protocol_common: ProtocolCommon | None = Field(
        None,
