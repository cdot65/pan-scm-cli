...
class InterfaceAddress(BaseModel):
    ...
    floating_ip: str | None = Field(None, description="Floating IP address")
...
class DynamicIpAndPortInterfaceAddress(BaseModel):
    ...
    interface: str = Field(..., description="Interface name")
    ip: str | None = Field(None, description="Translated source IP address")
    floating_ip: str | None = Field(None, description="Floating IP address")
...
class StaticIp(BaseModel):
    ...
    bi_directional: BiDirectional | None = Field(
        None,
...
class DynamicIpAndPort(BaseModel):
    ...
    translated_address: list[str] | None = Field(None, description="Translated source IP addresses")
    interface_address: InterfaceAddress | None = Field(None, description="Translated source interface")
    ...
    def validate_dynamic_ip_and_port(self) -> "DynamicIpAndPort":
        ...
        if bool(self.translated_address) == bool(self.interface_address):
            raise ValueError("Either translated_address or interface_address must be provided, but not both")
        return self
...
class SourceTranslation(BaseModel):
    ...
    dynamic_ip_and_port: DynamicIpAndPort | None = Field(None, description="Dynamic IP and port translation configuration")
    dynamic_ip: DynamicIp | None = Field(None, description="Dynamic IP translation configuration")
    static_ip: StaticIp | None = Field(None, description="Static IP translation configuration")
    ...
    def validate_source_translation(self) -> "SourceTranslation":
        ...
        if len(provided_types) != 1:
            raise ValueError("Exactly one of dynamic_ip_and_port, dynamic_ip, or static_ip must be provided")
        return self
...
class DestinationTranslation(BaseModel):
    ...
    translated_address: str | None = Field(None, description="Translated destination IP address")
    translated_port: int | None = Field(
        None,
        description="Translated destination port",
        ge=1,
        le=65535,
    )
    dns_rewrite: DnsRewrite | None = Field(None, description="DNS rewrite configuration")
...
class NatRuleBaseModel(BaseModel):
    ...
    source_translation: SourceTranslation | None = Field(
        None,
        description="Source translation configuration",
    )
    destination_translation: DestinationTranslation | None = Field(
        None,
    ...
    active_active_device_binding: str | None = Field(
        None,
    ...
    def validate_nat64_dns_rewrite_compatibility(self) -> "NatRuleBaseModel":
        ...
        if self.nat_type == NatType.nat64 and self.destination_translation and self.destination_translation.dns_rewrite:
    ...
    def validate_bidirectional_nat_compatibility(self) -> "NatRuleBaseModel":
        ...
        if self.source_translation and self.source_translation.static_ip and self.source_translation.static_ip.bi_directional == BiDirectional.YES and self.destination_translation:
...
class NatRuleMoveModel(BaseModel):
    ...
    destination_rule: UUID | None = Field(
        None,
    ...
    def validate_move_configuration(self) -> "NatRuleMoveModel":
        ...
        if self.destination in (NatMoveDestination.BEFORE, NatMoveDestination.AFTER):
            if not self.destination_rule:
                raise ValueError(f"destination_rule is required when destination is '{self.destination.value}'")
        else:
            if self.destination_rule is not None:
                raise ValueError(f"destination_rule should not be provided when destination is '{self.destination.value}'")
        return self
