...
class MatchListItem(BaseModel):
    ...
    action_desc: str | None = Field(None, description="Match profile description", max_length=255)
    log_type: Literal[
        "traffic",
        "threat",
        "wildfire",
    ...
    filter: str | None = Field(None, description="Filter match criteria", max_length=65535)
    send_http: list[str] | None = Field(None, description="A list of HTTP server profiles")
    send_syslog: list[str] | None = Field(None, description="A list of syslog server profiles")
    send_to_panorama: bool | None = Field(None, description="Flag to send logs to Panorama")
    quarantine: bool | None = Field(False, description="Flag to quarantine matching logs")
...
class LogForwardingProfileBaseModel(BaseModel):
    ...
    match_list: list[MatchListItem] | None = Field(
        None,
        description="List of match profile configurations",
    )

    enhanced_application_logging: bool | None = Field(
        None,
