...
class AddressBaseModel(BaseModel):
    ...
    tag: list[TagString] | None = Field(  # type: ignore
        None,
    ...
    def validate_address_type(self) -> "AddressBaseModel":
        ...
        if len(provided) == 0:
            raise ValueError("Value error, Exactly one of 'ip_netmask', 'ip_range', 'ip_wildcard', or 'fqdn' must be provided.")
        elif len(provided) > 1:
