...
class BGPRoutingBaseModel(BaseModel):
    ...
    routing_preference: DefaultRoutingModel | HotPotatoRoutingModel | None = None
    backbone_routing: BackboneRoutingEnum | None = None
    accept_route_over_SC: bool | None = None
    outbound_routes_for_services: list[str] | None = None
    add_host_route_to_ike_peer: bool | None = None
    withdraw_static_route: bool | None = None
    ...
    def serialize_routing_preference(self, value: DefaultRoutingModel | HotPotatoRoutingModel | None) -> dict[str, Any] | None:
...
class BGPRoutingCreateModel(BGPRoutingBaseModel):
    ...
    routing_preference: DefaultRoutingModel | HotPotatoRoutingModel | None = Field(
        None,
    ...
    def validate_routing_preference_type(self) -> "BGPRoutingCreateModel":
        ...
        if self.routing_preference is not None:
            if not isinstance(self.routing_preference, (DefaultRoutingModel, HotPotatoRoutingModel)):
                raise ValueError("routing_preference must be either DefaultRoutingModel or HotPotatoRoutingModel")
        return self
...
class BGPRoutingUpdateModel(BGPRoutingBaseModel):
    ...
    backbone_routing: BackboneRoutingEnum | None = Field(
        None,
        description="Backbone routing configuration for asymmetric routing options",
    )

    accept_route_over_SC: bool | None = Field(
        None,
        description="Accept route over Service Connection",
    )

    outbound_routes_for_services: list[str] | None = Field(
        None,
        description="List of outbound routes for services in CIDR notation",
    )

    add_host_route_to_ike_peer: bool | None = Field(
        None,
        description="Whether to add host route to IKE peer",
    )

    withdraw_static_route: bool | None = Field(
        None,
        description="Whether to withdraw static route",
    )
...
class BGPRoutingResponseModel(BGPRoutingBaseModel):
    ...
    routing_preference: DefaultRoutingModel | HotPotatoRoutingModel = Field(
        ...,
        description="The routing preference configuration",
    )
