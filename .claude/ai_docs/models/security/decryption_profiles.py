...
class SSLForwardProxy(BaseModel):
    ...
    block_tls13_downgrade_no_resource: bool = Field(False, description="Block TLS 1.3 downgrade when no resource is available,")
...
class DecryptionProfileBaseModel(BaseModel):
    ...
    ssl_forward_proxy: SSLForwardProxy | None = Field(
        None,
        description="SSL Forward Proxy settings",
    )
    ssl_inbound_proxy: SSLInboundProxy | None = Field(
        None,
        description="SSL Inbound Proxy settings",
    )
    ssl_no_proxy: SSLNoProxy | None = Field(
        None,
        description="SSL No Proxy settings",
    )
    ssl_protocol_settings: SSLProtocolSettings | None = Field(
        None,
